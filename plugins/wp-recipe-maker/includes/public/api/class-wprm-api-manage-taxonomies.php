...
class WPRM_Api_Manage_Taxonomies {
	...
	public static function api_manage_taxonomies( $request ) {
		...
		if ( 'ingredient' === $type ) {
			...
			foreach( $rows as $index => $row ) {
				// EAFL.
				$row->eafl = get_term_meta( $row->term_id, 'wprmp_term_eafl', true );
				...
				// Ingredient link.
				$row->link = get_term_meta( $row->term_id, 'wprmp_term_link', true );
				$row->link_nofollow = '';
				if ( $row->link ) {
					$link_nofollow = get_term_meta( $row->term_id, 'wprmp_term_link_nofollow', true );
					$row->link_nofollow = in_array( $link_nofollow, array( 'default', 'nofollow', 'follow' ) ) ? $link_nofollow : 'default';
				}
